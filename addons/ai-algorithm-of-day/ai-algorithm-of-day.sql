-- AI Algorithm of the Day — reference registration + schema (mirrored from the ilm.red monorepo).
...
VALUES (
  'ai-algorithm-of-day', 'AI Algorithm of the Day', '1.0.0', 'A different machine-learning algorithm or method each day, with a plain-words explainer. Curated catalogue, refreshed daily.', '🧠',
  'ILM Red Unbound', 'Apache-2.0', 'AiAlgorithmOfDayAddon', 'engagement', ARRAY[]::text[],
  '{}'::jsonb, false, true, true, 'published', 'Shows one item a day from a curated catalogue of ML algorithms. An optional AI-written explainer is generated server-side. No personal data is collected.')
...
WHERE bc.id = :club_id
  AND ar.slug = 'ai-algorithm-of-day'
