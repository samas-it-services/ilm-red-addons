-- Math Equation of the Day — reference registration + schema (mirrored from the ilm.red monorepo).
...
VALUES (
  'math-equation-of-day', 'Math Equation of the Day', '1.0.0', 'A famous equation each day, beautifully rendered, with a note on why it matters. Curated catalogue, refreshed daily.', '➗',
  'ILM Red Unbound', 'Apache-2.0', 'MathEquationOfDayAddon', 'engagement', ARRAY[]::text[],
  '{}'::jsonb, false, true, true, 'published', 'Shows one famous equation a day from a curated catalogue, rendered with KaTeX. An optional AI-written note is generated server-side. No personal data is collected.')
...
WHERE bc.id = :club_id
  AND ar.slug = 'math-equation-of-day'
